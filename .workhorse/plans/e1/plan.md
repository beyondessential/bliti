...
- [x] Remove the hand-written page and script: `crates/bliti-web/www/index.html` and `crates/bliti-web/www/app.js`.
- [x] Remove the send-text demonstration message and the device-side print behind it:
...
- [x] Remove `DeviceMessage::Unknown` and `parse_client_message`'s error-reply behaviour, replaced by the three outcomes below. Nothing is answered on the wire any more.
...
- [x] **Delimiting.** Application messages on a yamux stream are length-delimited with a four-byte big-endian prefix, which `write_message`/`read_message` in `crates/bliti-core/src/channel/stream.rs` already do. Add the one-mebibyte ceiling: `read_message` currently reads a `u32` length and allocates it unbounded, so it needs the same refusal `Reassembler` gives at the Noise layer, closing the stream rather than the connection.
- [x] **Stream roles.** Device opens its reporting stream on connect and sends `device-hello` first (it already opens one and sends `Identity`; the hello goes in front). Client opens a control stream on connect and sends `client-hello` first. Roles come from who opened the stream and its first message, never from the stream id.
- [x] **Hello messages.** Both carry `name` and `version` as opaque strings. Nothing parses, compares, or orders them: the client displays the device's, the device logs the client's. Keeping them opaque is what makes the no-gate rule enforceable rather than merely intended.
- [x] **Member names and criticality.** Names are lower case letters, digits and hyphens, matched case-insensitively, wholly lower or wholly upper. Upper case marks the member critical. Mixed case, or the same name twice in any casing, is malformed. Applies at every depth, to names only and never to values.
- [x] **Three outcomes, not two.** The parse layer must keep these apart, because collapsing any two of them loses the property the envelope exists for:
...
- [x] **Parsing.** `#[serde(tag = "type", rename_all = "kebab-case")]` cannot express any of this: it is case-sensitive, cannot report which unknown members it saw, and errors where we need three distinct outcomes. Parse in two stages instead. First read to a `serde_json::Map`, validate each name (well formed, not duplicated case-insensitively), and record which arrived upper case while lowering every key. Then deserialise the normalised object into the typed struct, with `#[serde(flatten)] unknown: Map<String, Value>` capturing what the struct does not know. Any captured name that arrived upper case is a critical unknown; the rest are ignorable. Recurse for nested objects.
- [x] **Growth rule.** Adding a member is a lower-case name if the message still means something without it, an upper-case name if it does not. Neither needs a base version bump. Removing a member, or changing what one means, does.
- [x] **Client display.** A critical unknown is shown to the operator as "this device said something this version of the app is too old to act on", with everything else still rendered. It must not blank the view, and must not look like a fault.
- [x] **Subscription streams.** A client subscribes by opening a stream whose first message is `subscribe` with a `topic` string; the device sends that topic's data on that same stream; the client unsubscribes by closing the stream. There is no `unsubscribe` message. One stream per subscription.
- [x] **End of stream is the unsubscribe, device side.** This is the load-bearing half. `close()` sends FIN and leaves the stream `SendClosed`, which yamux documents as "open for incoming messages" (`connection/stream.rs:57`): it is a half-close, so the device's write side stays open and the device will keep streaming unless it acts on the EOF. On the device, `read_message` returning `Ok(None)` on a subscription stream must stop the topic, close the device's side, and drop the subscription. Without this the page-visibility case silently does nothing.
- [x] **Any termination unsubscribes.** A reset counts as well as a FIN: dropping a `Stream` in state `Open` sends RST (`connection.rs:516-529`), so a dropped or abandoned stream does reach the device. Handle reset, EOF, and connection loss through the same path, so no drop guard is needed and no client-side discipline is relied on. Prefer an explicit `close()` anyway, so the ordinary path is a graceful FIN rather than a reset.
- [x] **In-flight data.** Samples already queued when the stream ends may still arrive at the client; discard them rather than erroring.
- [x] **Unknown topic.** A device that does not know a topic skips the `subscribe` like any unrecognised message and sends nothing, leaving the stream open and empty. This is the older-device path and it must not error.
- [x] **Page visibility.** The client closes its subscription streams on page-hidden and opens fresh ones on page-shown. Lives in the React app.
- [x] **No gate above the base version.** The base protocol version in the advertisement is the one version that is switched on, and it covers the crypto, the exchange, and the fact that messages are JSON. The checks in `Sticker::new`/`read_local_name` are that gate and stay. Above it, nothing compares a version.
...
Settled: each end reports the name and version of the package it was built from. The device and the
command-line client use `CARGO_PKG_NAME` and `CARGO_PKG_VERSION`, and the application uses its
`package.json` name and version, threaded in at build time so what is reported is what was shipped.
Both are opaque to the other end.
...
- [x] Scaffold a vanilla Vite + React app (npm) under `crates/bliti-web/` (app sources alongside the wasm crate). Decide the exact layout: the wasm crate's `src/`/`Cargo.toml` stay; the JS app and its `package.json` sit beside them.
- [x] Wire the wasm module in: `build.sh` already emits `wasm-bindgen --target web` output; have Vite consume the generated ES module and bindings rather than the browser importing them directly.
- [x] Port the prototype's real behaviour into React: read a sticker by link fragment and by camera, scan and match, open the channel, drive Web Bluetooth, and render a device view (identity, addresses, device version) with no readings yet.
- [x] The React half drives Web Bluetooth, the camera, and the interface; the wasm half stays the protocol. The boundary does not move.
- [x] Graphs are drawn as SVG from the sample buffer with no charting dependency — but the buffer and the readings are D1, so E1 only leaves room for them.
...
- [x] A Playwright harness that fakes at the message layer: decoded messages fed directly to the app, no wasm and no BLE in the loop. It covers the view, the tile/tap behaviour (D1), and the envelope lifecycle (subscribe/unsubscribe around page visibility, skip-unknowns, version display).
- [x] Nothing tries to fake Web Bluetooth. Protocol and transport coverage stays in Rust tests. Bluetooth is exercised manually against real hardware.
