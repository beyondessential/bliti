...
- [x] **Subscription streams.** A client subscribes by opening a stream whose first message is `subscribe` carrying `TOPIC`, its one pinned-critical member; the device sends that topic's data on that same stream; the client unsubscribes by closing the stream. There is no `unsubscribe` message. One stream per subscription.
