...
- [ ] Check whether capabilities offer the hotspot channels the firmware refuses. On the Pi under NZ, brcmfmac fails `set_channel` for 34 to 46 and 144 to 165 (see "A hotspot first on the shared-channel radio drops a 5 GHz join" below), while the probe builds channels from cfg80211's flags, which permit some of them. A hotspot proposed on one would fail to start. If so, find what the firmware accepts (its own country code, `99` today, not following the domain set) and either set it or leave those channels out
