...
- [ ] Wireless joining: PSK, SAE, transitional, enterprise, WPS push-button and PIN
  - [ ] A wireless candidate out of range is never tried again: with every network `AutoConnect=false` iwd does not scan by itself, and a retry of a candidate the radio does not hear changes nothing, so a network coming into range goes unnoticed. LINK forbids polling to detect it, so this needs a decision on what scans
  - [ ] Reasons from iwd reach the client in iwd's words (`Operation failed (net.connman.iwd.Failed)` for a wrong passphrase); they want to be in terms an operator can act on
  - [ ] An apply connects from iwd's cached scan results while its own scan runs, so a network that has just gone away is tried and fails with status 16 before the scan says it is out of range
