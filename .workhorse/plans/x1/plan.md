...
- [x] WPS for a chosen network: `wps` takes an optional `ssid` (CFG, WLAN); the device runs WPS as now and accepts the credentials only where they are for that SSID, discarding them and reporting the mismatch otherwise; the screen offers joining by WPS from a network in the scan list (NSCR). iwd's `SimpleConfiguration` takes no network or BSSID (`PushButton()`, `StartPin(s)`, checked on iwd 3.10), so the choice is enforced on the result rather than on the exchange. Push-button's session-overlap rule already aborts where two access points have the button pressed at once, and a PIN is targeted by the router it is entered on. Done: a mismatch disconnects the station and forgets the network through iwd's `KnownNetwork.Forget`, removing its file where iwd left one, while the radio is still held, before the session restores; it is `invalid` at `$['ssid']` with no `reached`, since no candidate's verification failed. `ssid` stays an ordinary member rather than a critical one, as `interface` is: a client checks it against `acts` first. The screen offers WPS only on a network the device can join, and joins on the adapter the candidate being edited is pinned to; as with the WPS buttons under Add, the device's proposal replaces edits not applied. Not checked on hardware
...
- [x] A hotspot on a shared-channel radio chooses its own band, channel and width where no wireless candidate can take that radio, since there is then no client to follow. HOT forbids it today, omitting the three from the radio's capabilities outright. Needs: HOT rewritten to offer them conditionally; the capabilities to carry them with that condition (the mirror has no selector for it, so a cross-member rule beside the one-at-a-time placement rule, in `bliti-core` for device and client both); the renderer to use a chosen channel; the screen to offer them then and explain their absence once a wireless connection shares the radio. Do with the bring-up ordering below, which is the same code. A candidate counts as able to take the radio where it names that radio or names none, whatever its `bands`; the screen takes the three out of the hotspot once one could, rather than leaving a setting it no longer shows
- [x] Order the shared-channel radio's bring-up as HOT has it, so the hotspot follows the wireless client: settle the station first, then start or move the hotspot onto its channel. The hotspot now waits, unrendered, while its radio's client is at or before association, and while a proposal's scan of that radio is out with a wireless candidate that could take it; it then starts once, on the client's channel, or on its own where no client joins
  - [x] iwd's autoconnect was not racing the driver. Every network renders `AutoConnect=false`, and iwd logs `autoconnect_quick` and `autoconnect_full` as its idle states whatever the setting; the join in that run was the driver's own, 6 seconds after iwd restarted. The drop was hostapd: the first deauthentication came while hostapd held 2.4 GHz channel 6 with the station on 5 GHz, and the second 0.4 seconds after the driver stopped hostapd to restart it on the station's channel. On brcmfmac, taking the access point down takes the station with it, which the ordering above avoids
  - [x] A client on a channel no access point may start on (hostapd refuses 5 GHz channel 140 on the Pi, radar) leaves the hotspot no channel. The driver holds it back and fails the proposal at `$['hotspot']` saying which channel and why, rather than letting hostapd fail. HOT needs a sentence for it once the shared-channel section settles
  - [ ] Check on the Pi that starting hostapd beside an associated station on its channel keeps the station up
