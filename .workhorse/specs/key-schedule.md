...
Two derivations take the [board ID](overview.md#board-id) of [BID](board-id.md) to the [presence token](overview.md#presence-token) carried in the QR code and the [advertised handle](overview.md#advertised-handle) broadcast over BLE.

Both derivation constants are public, and are compiled into devices, generators and clients.

> [!NOTE]
> Publishing the constants weakens nothing, because neither derivation runs backwards. What they provide is domain separation, so that a value from one step is not a valid value at another.
...
The presence token MUST be derived with argon2id, version `0x13`, over:
...
> [!NOTE]
> Changing any parameter produces a different token and orphans every QR code already printed under the old ones.
...
A device MUST derive its own presence token.

A device MUST cache the derived token together with the board ID it was derived from, the platform serial of the board it was derived on, and which kind of source won the precedence.
...
- where the serial and the strongest kind present both match the cache, the cached token stands, and the device MUST NOT read a source value or run a derivation
