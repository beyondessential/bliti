...
- [x] `topic` and `TOPIC` reach the same member, and a receiver that knows it behaves identically for both (Rust).
- [x] A mixed-case name is malformed and takes the fault path (Rust).
- [x] The same name twice, in any combination of casings, is malformed and takes the fault path (Rust).
- [x] Casing applies to names and not values: a `topic` of `SYSTEM` is a different topic from `system`, not the same one marked critical (Rust).

### Ignorable unknowns are skipped

- [x] An unrecognised lower-case member is skipped and the rest of the object is read (Rust, both directions).
- [x] A message whose `type` the receiver does not recognise is skipped whole, with nothing sent in reply and the stream left open (Rust, both directions).
- [x] A client skips unrecognised lower-case members and unknown types from the device, and the view carries on (Playwright).
- [x] Skipping never closes the stream or the connection.

### Critical unknowns are refused, not faulted

- [x] An unrecognised upper-case member means the object carrying it is not processed (Rust, both directions).
- [x] The stream stays open and the connection is untouched: this is a newer peer, not a broken one (Rust). Regression guard against collapsing this into the fault path.
...
- [x] A `device-hello` carrying an unrecognised critical member leaves the client without the device name and version, saying so and carrying on with the session rather than refusing it (Playwright).
...
- [x] Bytes that are not valid UTF-8, not valid JSON, or JSON that is not an object are reported and close the stream they arrived on (Rust).
...
- [x] A message is read back byte-identical when its four-byte big-endian prefix is split across stream reads (Rust).
- [x] Several messages in one read are separated correctly (Rust).
...
- [x] Closing the stream ends the subscription and the device sends nothing further (Rust).
- [x] The device acts on end of stream rather than waiting for its own side to close: after the client half-closes, the device stops sending and closes its side (Rust). This is the case a half-close silently breaks.
- [x] A stream reset, rather than a graceful close, ends the subscription the same way (Rust).
...
- [x] Data in flight when the stream ends is discarded by the client rather than treated as a fault (Playwright).
...
- [x] No send-text affordance in the client, and no device-side print of client text.
- [x] The `www/index.html` and `www/app.js` prototype no longer exist; the app is the built bundle.
...
- [x] CI builds the static bundle (wasm + app) and keeps it as an artefact.
